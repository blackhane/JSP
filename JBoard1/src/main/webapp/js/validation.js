...
$(function(){
	...
	$('input[name=name]').focusout(function(){
		...
		if(!name.match(regName)){
			isNameOk = false;
			$('.resultName').css('color','red').text('한글 2자 이상이어야 합니다.');
		}else{
			isNameOk = true;
			$('.resultName').text('');
		}
	...
	$('#btnNickCheck').click(function(){
		...
		if(!nick.match(regNick)){
			isNickOk = false;
			$('.resultNick').css('color','red').text('유효하지 않는 별명입니다.');
			return;
		}
	...
	$('input[name=email]').focusout(function(){
		...
		if(!email.match(regMail)){
			isEmailOk = false;
			$('.resultEmail').css('color','red').text('올바른 형식의 이메일을 입력해주세요.');
		}else{
			isEmailOk = true;
			$('.resultEmail').text('');
		}
	...
	$('input[name=hp]').focusout(function(){
		...
		if(!hp.match(regHp)){
			isHpOk = false;
			$('.resultHp').css('color','red').text('올바른 형식의 휴대폰번호를 입력해주세요.');
		}else{
			isHpOk = true;
			$('.resultHp').text('');
		}
